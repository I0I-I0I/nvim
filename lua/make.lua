...
vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function(args)
        ...
        vim.bo[args.buf].makeprg = "ruff check --output-format=pylint"
        vim.bo[args.buf].errorformat = "%f:%l: %m"
        -- Format selection with 'gq'
        vim.bo[args.buf].formatprg = "ruff format --stdin-filename %"
        vim.bo[args.buf].formatexpr = ""
        vim.keymap.set("n", "<leader>f", "<cmd>w<cr><cmd>!ruff format %<cr>",
            { silent = true })
        vim.keymap.set("n", "<leader>M", "<cmd>w<cr><cmd>silent !ruff check --fix % >/dev/null<cr>",
            { silent = true })
    end })
